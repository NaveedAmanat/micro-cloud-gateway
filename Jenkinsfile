pipeline{
    ...
    stages{
        stage('Build Maven'){
            steps{
                checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/NaveedAmanat/micro-cloud-gateway.git']]])
        ...
        stage('Trigger Manifest'){
            steps{
                build job: 'micro-cloud-gateway-artifact', parameters: [string(name: 'IMAGE_TAG', value: env.BUILD_NUMBER)]
            }
        }
    }
}
